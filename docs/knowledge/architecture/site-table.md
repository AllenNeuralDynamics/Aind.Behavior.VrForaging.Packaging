...
- **Choice with no in-window `IsStopped=True`**: falls back to a global search
  before the choice time (or raises if `strict_parsing`). If the global search
...
logic and honour `strict_parsing` through it. The two `IsStopped` branches above
